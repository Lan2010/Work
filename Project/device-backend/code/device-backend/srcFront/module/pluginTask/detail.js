...
define(['jquery','Global','Menu','Grid'],function($,G,Menu,Grid){
	...
	var getDetail = function(fn){
		...
		var uri = '/api/ipk/ipkTaskDetailById';

		$.ajax({
			url: G.path + uri,
			data: {ipkTaskId: opts.id},
			cache: false,
			success: function(json){
				...
				$form.fillForm(data,{
					fields: [
						...
						'ipkNickName',
						{'operateType': data.operateType == 0 && '升级' || data.operateType == 1 && '卸载' || '' },
						'version',
						{'append': data.operateType == 0 ? data.append.replace(/--force-/g,'') : ''},
						'taskDesc',
