...
define(['jquery','Global','Menu','Validate','Grid','DTP'],function($,G,Menu,Validate,Grid){
	...
	var setSearch = function(){
		...
		$formSearch.on('submit',function(){
			...
			if ($('#ipkNameId').val() == '' || $('#version').val() == ''){

				G.$page.msg({text: '请先选择升级的插件和版本',time: 3000});
				return false;
			}
	...
	var setOptions = function(){
		...
		// 升级配置 - 插件、版本
		var uri_p = G.path + '/api/ipk/ipkNamelist',
			uri_v = G.path + '/api/ipk/queryVersion';

		$("#ipkNameId").setSelect({
			url: uri_p, id: 'ipkNameId', name: 'ipkNickName', relate: ['#version'],
			onChange: function(v1,d1){
				...
				$("#version").setSelect({
					url: uri_v, data: {ipkNameId: v1}, id: 'version', name: 'version',
	...
	var setForm = function(data){
		...
		$form.on('submit',function(){
			...
			var uri = selectType == 1 ? '/api/ipk/addIpkTaskBySearch' : '/api/ipk/addIpkTaskByDevids';
