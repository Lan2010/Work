...
define(['jquery','Global','Menu','Validate','Grid','DTP'],function($,G,Menu,Validate,Grid){
	...
	var setSearch = function(){
		...
		$formSearch.on('submit',function(){
			...
			if ($('#firmwareNameId').val() == '' || $('#version').val() == ''){

				G.$page.msg({text: '请先选择更新的固件和版本',time: 3000});
				return false;
			}
	...
	var setOptions = function(){
		...
		// 任务配置 - 固件、版本
		var uri_p = G.path + '/api/firmware/firmwareNamelist',
			uri_v = G.path + '/api/firmware/queryVersion';

		$("#firmwareNameId").setSelect({
			url: uri_p, id: 'firmwareNameId', name: 'nickName', relate: ['#version'],
			onChange: function(v1,d1){

				// Search param
				$('#s_firmwareNameId').val(v1);

				resetTable();

				$("#version").setSelect({
					url: uri_v, data: {firmwareNameId: v1}, id: 'version', name: 'version',
					onChange: function(v2,d2){
	...
	var setForm = function(data){
		...
		$form.on('submit',function(){
			...
			var uri = selectType == 1 ? '/api/firmware/addTaskBySearch' : '/api/firmware/addTaskByDevids';
